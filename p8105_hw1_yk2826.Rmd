...
The proportion of cases for which the logical vector is TRUE is `r sum(plot_df$vec_logical, na.rm = TRUE) / 1000`.


The chunk below makes scatterplots of y vs x; exports the first scatterplot.

```{r scatterplots}
ggplot(plot_df, aes(x = x, y = y)) + geom_point(aes(color = vec_logical))
ggsave("scatter_logical.pdf", height = 4, width = 6)
ggplot(plot_df, aes(x = x, y = y)) + geom_point(aes(color = vec_numeric)) 
ggplot(plot_df, aes(x = x, y = y)) + geom_point(aes(color = vec_factor)) 
```